...
@Component({
    ...
    standalone: true,
    imports: [InfiniteScrollDirective, FileItemComponent],
...
export class FilesOrFoldersTableComponent extends PagingBase<PastaOuArquivoPageItem> implements OnInit {
    ...
    set parameters(value: PastasParameter | undefined) {
        ...
        if (this.showTitle) this.getPastaProperties(value.id, value.rootId!);
        ...
        if (value.rootId) this.param.queryStrings.set('rootId', value.rootId);
