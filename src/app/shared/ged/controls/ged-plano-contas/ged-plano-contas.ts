...
export class GedPlanoContasComponent extends PagingBase<PastaPageItem> implements OnInit {
    ...
    set parameters(value: PastasParameter | undefined) {
        ...
        if (value.cadastroId) this.param.routeStrings.push(value.cadastroId.toString());

        if (value.userId) this.param.routeStrings.push(value.userId.toString());
        ...
        if (value.rootId) {
            ...
            console.log(value.rootId);
        }

        if (value.id) this.param.queryStrings.set('pastaId', value.id);
