...
export class ContabilUsersService extends ServiceBase {
    ...
    userClienteAdd(userCliente: UserClienteInput): Observable<ApiResponse<ClienteUserPageItem[]>> {
        return this.http.post<ApiResponse<ClienteUserPageItem[]>>(`${this.apisUtilsService.getApiUrl(TMicroService.ApiContabil)}/Users/UserClienteAdd`, [userCliente], this.httpService.httpOptions)
            .pipe(catchError(this.handleError))
    }

    contabilUserGet(id: number): Observable<ApiResponse<ContabilUserView>> {
        return this.http.get<ApiResponse<ContabilUserView>>(`${this.apisUtilsService.getApiUrl(TMicroService.ApiContabil)}/Users/UserGet/${id}/${this.vars.cadastro?.id}`)
            .pipe(catchError(this.handleError))
    }
